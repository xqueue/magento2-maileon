...
use Magento\Sales\Model\Order\Email\Sender\ShipmentSender as MagentoShipmentSender;
...
class ShipmentSender extends MaileonSender
{
    /**
     ...
     *
     * @param MagentoShipmentSender $subject
     * @param callable $proceed
     * @param Shipment $shipment
     * @param bool $forceSyncMode
     */
    public function aroundSend(
        MagentoShipmentSender $subject,
        callable $proceed,
        Shipment $shipment,
        $forceSyncMode = false
    ) {
        if ($this->pluginConfig['shipmentStatus'] == 'yes') {
            ...
            if ($this->checkAndSend($order, $shipment)) {
     ...
     * @return bool
     */
    protected function checkAndSend(Order $order, Shipment $shipment)
    {
        $this->identityContainer->setStore($order->getStore());
        ...
        if ($contactCreated) {
            ...
            return $transactionCreate->sendTransaction(
                ...
                $this->createTransactionContent($order, $shipment)
            );
        }
     ...
     * @return array
     */
    protected function createTransactionContent(Order $order, Shipment $shipment): array
    {
        $content = [];
