...
namespace Maileon\SyncPlugin\Vendor\MaileonAPI\json;
...
abstract class AbstractJSONWrapper {
    ...
    function toArray() {
        ...
        foreach($object_vars as $key => $value) {
            ...
            if(gettype($value) == "object" &&
             is_subclass_of($value, 'Maileon\SyncPlugin\Vendor\MaileonAPI\json\AbstractJSONWrapper')) {
                if(!$value->isEmpty()) {
    ...
    function fromArray($object_vars) {
        ...
        foreach($object_vars as $key => $value) {
            if(class_exists('Maileon\SyncPlugin\Vendor\MaileonAPI\json\AbstractJSONWrapper') && is_subclass_of( $this->{$key},'Maileon\SyncPlugin\Vendor\MaileonAPI\json\AbstractJSONWrapper' )) {
