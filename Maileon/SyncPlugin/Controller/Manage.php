...
namespace Maileon\SyncPlugin\Controller;

use Magento\Store\Model\ScopeInterface;
use Maileon\SyncPlugin\Model\Maileon\ContactCreate;
use Maileon\SyncPlugin\Vendor\MaileonAPI\utils\PingService;
        ...
        if ($module_enabled == 'yes') {
            if ((boolean)$this->request->getParam('is_subscribed', false)) {
                ...
                try {
                    ...
                    $response = $sync->unsubscribeMalieonContact();
                } catch (\Magento\Framework\Exception\LocalizedException $e) {
                    $this->messageManager->addException(
                        $e,
                        __('There was a problem with the Maileon unsubscription: %1', $e->getMessage())
                    );
                }
            }
        } else {
