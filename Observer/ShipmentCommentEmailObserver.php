...
class ShipmentCommentEmailObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer): void
    {
        try {
            ...
            if ($this->transactionHelper->updateOrCreateContactFromOrder($order)) {
                ...
                $content = $this->addStoreDataToTransactionContent($order, $comment, $shipment, $content);

                $transactionService->sendTransaction(
                    $order->getCustomerEmail(),
                    Config::ORDER_SHIPMENT_UPDATED_TX_NAME,
                    $content
                );
            }
