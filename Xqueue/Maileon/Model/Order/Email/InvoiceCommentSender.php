...
use Magento\Sales\Model\Order\Email\Sender\InvoiceCommentSender as MagentoInvoiceCommentSender;
...
class InvoiceCommentSender extends MaileonSender
{
    /**
     ...
     *
     * @param MagentoInvoiceCommentSender $subject
    ...
    public function aroundSend(
        MagentoInvoiceCommentSender $subject,
        callable $proceed,
        Invoice $invoice,
        $notify = true,
        $comment = ''
        ...
        if ($this->pluginConfig['invoice'] == 'yes') {
            ...
            $this->checkAndSend($order, $invoice, $comment);
     ...
     * @return bool
     */
    protected function checkAndSend(Order $order, Invoice $invoice, string $comment = '')
    {
        $this->identityContainer->setStore($order->getStore());
     ...
     * @return array
     */
    protected function createTransactionContent(Order $order, Invoice $invoice, string $comment = ''): array
    {
        $content = [];
